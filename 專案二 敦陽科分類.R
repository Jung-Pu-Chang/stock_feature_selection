...
library(glmnet)
library(MLmetrics)
敦陽科 <- fread(file="D:\\2.敦陽科.csv",header=TRUE,stringsAsFactors = FALSE)
#na踢掉
any_na(敦陽科)
敦陽科$進場 <- as.factor(敦陽科$進場)
敦陽科 <- mutate(敦陽科,年=year(年月日))
#照時間切，準確度都只有3~4成
#traindata <- subset(敦陽科,年<2018)
#traindata <- traindata[,-c(1:3,20)]
#testdata <- subset(敦陽科,年>=2018)
#testdata <- testdata[,-c(1:3,20)]
n <- nrow(敦陽科)
set.seed(1117)
random <- sample(seq_len(n), size = round(0.7 * n))
traindata <- 敦陽科[random,-c(1:3,20)]
testdata <- 敦陽科[-random,-c(1:3,20)]
...
varImpPlot(rf_model)
#write.table(rf_final, file="D:\\敦陽科_2018後.csv", sep = ",", na = "", row.names=FALSE, col.names = TRUE)
...
library(Rdimtools)
敦陽科.dat = as.matrix(敦陽科[,4:18])
敦陽科.lab = as.factor(敦陽科[,19])
fsocre = do.fscore(敦陽科.dat, 敦陽科.lab, ndim = 8)
#自變量(彼此需獨立),應變量(類別型), ndim看前幾名
